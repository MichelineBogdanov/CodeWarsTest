...
/*
  ...
  If we continue working trying to find the successive values of Ni for our starting integer N = 26 we will find the
  following chain.
  ...
  Special Cases
  If your starting integer is less than 5, we do not have a result higher than your starting integer. In those cases
  your code should output -1. As an example:
  input:4; output:-1
  You will see more examples in the fixed examples of the sample box. You have to pass 200 random tests and the values
  for the starting N are in the following range:
  10 < N < 1 x 10^9
...
public class MaxNumChain {

    public static void main(String[] args) {
        System.out.println(maxIntChain(6));
        System.out.println(maxIntChain(10));
        System.out.println(maxIntChain(26));
        System.out.println(maxIntChain(32));
        System.out.println(maxIntChain(7));
    }

    public static long maxIntChain(long n) {
        if (n < 5) return -1;
        return n % 2 == 0 ? (n / 2 + 1) * (n / 2 - 1) : (n / 2) * (n / 2 + 1);
    }
